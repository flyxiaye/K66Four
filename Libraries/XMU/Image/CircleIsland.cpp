		...
		case 6:
			if (CL == CircleFlag && 0 == RightLost)
			{
				...
				while (1)
				{
					...
					else if (OldRow <= DOWN_EAGE - FIVE_SIX_TH - 5)
					{
						if (IndJudgeCircle(0))
							CircleState = 7;
						break;
	...
	/*	if (5 == CircleState)
		{
			if (CL == CircleFlag)
			{
				if (RL[DOWN_EAGE] > RIGHT_EAGE - 3)
				{
					for (int i = DOWN_EAGE - 1; i > DOWN_EAGE - 15; i--)
					{
						int tmp_col = SearchRightEage(i, 158);
						if (tmp_col < RIGHT_EAGE - 3)
							break;
					}
				}
			}
...
void GetPointC(void)
{
	...
	case 3:
		...
		//{
		//	PointC.Row = SearchUpEage(PointB.Row - 1, PointB.Col + 1);
