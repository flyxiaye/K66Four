...
void ImgJudgeStopLine(void)
{
	...
	if (Img_StopOpen && !Img_SpecialElemFlag
		&& LeftIntLine < UP_EAGE + 10 && RightIntLine < UP_EAGE + 10
		&& LeftPnt.ErrRow <= UP_EAGE + 8 && RightPnt.ErrRow <= UP_EAGE + 8
		&& !Img_StopLineFlag && DistStopLine(UP_EAGE + 8))
	{
		Img_StopLineFlag = 1;
...
void ImgJudgeObstacle(void)
{
	...
	if (g_inf > stop_inf)
	{
			...
			&& RightPnt.ErrCol - LeftPnt.ErrCol > 20)
		{
			...
			if (1 == Img_BlockOpen && !Img_SpecialElemFlag
...
{
	if (ImgJudgeSpecialLine(left_line, LL[left_line], right_line, RL[right_line], 0, 33))
	{
...
//================================================================//
//  @brief  :		内跳线判断封闭曲线
//  @param  :		左线行数 右线行数 type 0为直道断路 1位弯道断路 start_line 识别截止行
//  @return :		1 确认有
//  @note   :		全局变量LL RL
//================================================================//
...
{
	if (!type &&						//直道
		(left_row < start_line && right_row < start_line
	...
	//	//统计上方灰度均值
	//	int ave_gray = RegionAveGray(MIN(left_row, right_row), left_col, right_col);
	//	if (!type && ave_gray > LightThreshold) return 0;
...
int ImgJudgeOutBroken(void)
{
	...
	{
		int BrokenGray = RegionAveGray(DOWN_EAGE - 1, MIDDLE - 30, MIDDLE + 30);
		int ProGray = RegionAveGray(ProSpect, MIDDLE - 20, MIDDLE + 20);
		if (BrokenGray < LightThreshold)
			return 1;
		else if (ProGray - BrokenGray > DarkThreshold)
			return 2;
		else
			return 0;
